...
function bubbleSort(nums = []) {
    for (let i = 0; i< nums.length; i++){   //loop through the entire array
        for (let j = 0; j < nums.length; j++){ // another loop to compare values
            if (nums[j] > nums[j +1]){  //if the current index is greater than the next index
                let temp = nums[j+ 1]   //swap elements
                nums[j + 1] = nums[j]
                nums[j] = temp
            }
        }
